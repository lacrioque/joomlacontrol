...
//define used methods
	WARN = function(){ CONFIG.verbose >= 0 && console.log.apply(console, arguments); },
	INFO = function(){ CONFIG.verbose >= 1 && console.log.apply(console, arguments); },
	DEBUG = function(){ CONFIG.verbose >= 2 && console.log.apply(console, arguments); },
	WARNLINE = function(text){ CONFIG.verbose >= 0 && process.stdout.write(text); },
	INFOLINE = function(text){ CONFIG.verbose >= 1 && process.stdout.write(text); },
	DEBUGLINE = function(text){ CONFIG.verbose >= 2 && process.stdout.write(text); },
	...
	getComponentInfo = function(componentPath){
		...
		fs.readFile(path.join(componentPath,xmlFile), function(err,data){
			if(err){DEBUG(err); def.reject(err);}
			parser.parseString(data, function(error,result){
				if(err){DEBUG(err); def.reject(err);}
				def.resolve(result.extension.name[0].toLowerCase());
			});
		});
		return def.promise;
	},
	readDirRecursive = function(directory){
	    ...
	    fs.readdir(thisdirectory, function(err,files){
			if(err){DEBUG("58", err); def.reject(err);}
			...
			q.allSettled(promiseArray).then(
				...
				function(err){DEBUG("71", err); def.reject(err);}
			).catch(DEBUG);
		});
	    return def.promise;
	},
	getComponent = function(){
		...
		return q.Promise(function(resolve, reject){
			...
			function(values){
			    ...
			    resolve({admin: values[0].value, site: values[1].value, media: values[2].value});
			},
			reject
		    );
		});
	...
	copyFile = function(filename,from,to,i,count){
		let def = q.defer();
			fs.copy(path.join(from,filename), path.join(to,filename), function(err){
				if(err) {def.reject(err);throw err;}
	...
	copyAdminPart = function(adminFiles, installation, componentName, component){
		...
		let promises =_.map(adminFiles, function(f,i){
			...
			DEBUG({from: from, filename: filename, to: to});
							...
							.then(function(){
								...
								copyMediaPart(pathObj.media,installationPath,componentName,componentPath)
									.then(function(){
										...
										def.resolve()
									}, function(err){def.reject(err); throw err; });
							}, console.log);
					},console.log);
			},console.log
		);
					...
					function(answer){
						if(answer===true){
							copyComponent(files).then(
								...
								function(err){ DEBUG(err);   rl.close();}
							);
						} else {
							DEBUG(files);
							closeRL && rl.close();
	...
	askForPermission = function(checkPermission){
		...
		if(checkPermission){
			...
			return q.Promise(function(res,err){res(true)});
		}
	},
	watchMenu = function(){
...
####`;};
	...
	watchFiles = function(){
		CONFIG.watcher = gaze(CONFIG.componentPath+"/**/**", function(err, watcher){
			if(err){ INFO(err); }
	...
	changeOccured = function(event, filepath){
		...
		if(isInAdminpath(filepath)){
			copyTargetAdminFile(filepath, installationPath, CONFIG.componentName, componentPath)
				.then(function(){DEBUG('copied');},DEBUG);
...
if(args.configFile){
	try{
	let configString = fs.readFileSync(args.configFile,{'encoding': 'utf8'});
	...
	CONFIG.verbose = configParsed.verbosity !== undefined ? configParsed.verbosity : CONFIG.verbose;
		...
		if(e){
			INFO("The config file seems to be wrong ore missing.");
			process.abort();
	...
	if(!args.installationPath || !args.componentPath){
		INFO("There has to be an installation and a component path.\nEither define them with -i and -o or use a config file");
		process.abort();
	}
